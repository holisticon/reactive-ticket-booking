...
import de.holisticon.showcase.ticketbooking.service.{ FactDatabaseAware, FactDatabase }
import FactDatabase.{ PutSuccessful, DeleteFailed, DeleteSuccessful }
import de.holisticon.showcase.ticketbooking.service.FactDatabase
...
trait DistributedDataCrudDirectives extends Directives with FactDatabaseAware {
  ...
  /**
   * Creates a route that accepts post request with an entity of E and stores them
   * as DistributedData entity.
   * @param key the distributed data key
   * @param id the id of the entity
   * @tparam E type of the entity
   * @return
   */
  def createWithId[E: FromEntityUnmarshaller](key: LWWMapKey[E], id: => Any): Route = {
    post {
      entity(as[E]) { entity: E =>
