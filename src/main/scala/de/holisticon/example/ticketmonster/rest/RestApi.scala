...
import de.holisticon.example.ticketmonster.service.ClusterService.EventStream
import de.holisticon.example.ticketmonster.service.PerformanceBookingNode.NotEnoughSeats
  ...
  val metricsRouting: Route = {
    (path("metrics" ~ Slash.?) & get) {
      optionalHeaderValueByName("Last-Event-ID") { lastEventIdOpt =>
        complete {
          val fut = (clusterServiceRef ? ClusterService.Register(lastEventIdOpt.map(_.toLong))).mapTo[EventStream].map(_.stream)
          val futfut = fut.map(_.map { case (offset: Long, nm: NodeMetrics) => ServerSentEvent(NodeMetricsProtocol.write(nm).compactPrint, None, Some(offset.toString)) })
          futfut
        }
      }
