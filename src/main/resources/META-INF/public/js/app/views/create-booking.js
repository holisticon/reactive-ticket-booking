define([
	...
	'require',
	'configuration',
	'text!../../../../templates/booking-confirmation.html',
	'text!../../../../templates/create-booking.html',
	'text!../../../../templates/ticket-categories.html',
	'text!../../../../templates/ticket-summary-view.html',
	'bootstrap'
], function(utilities,
			require,
			config,
	...
	var CreateBookingView = Backbone.View.extend({
		...
		render: function () {
			...
			$.getJSON(config.baseUrl + "rest/shows/" + this.model.showId, function (selectedShow) {
				...
				self.ticketCategoriesView = new TicketCategoriesView({
					model: {},
		...
		save: function (event) {
			...
			bookingRequest.performance = parseInt(this.model.performanceId);
			bookingRequest.show = parseInt(this.model.showId);
			...
			$.ajax({
				...
				success: function (booking) {
					this.model = {};
					$.getJSON(config.baseUrl + 'rest/shows/' + bookingRequest.show + '/performance/' + booking.performance.id, function (retrievedPerformance) {
				...
				if (error.status == 400 || error.status == 409) {
					...
					$("#request-summary").append('<div class="alert"><button type="button" class="close" data-dismiss="alert">&times;</button><strong>Ouch! </strong>The server had a hiccup. Please try again!</div>');
				} else {
		...
		addQuantities: function () {
			...
			_.each(this.ticketCategoriesView.model, function (model) {
				if (model.quantity !== undefined) {
					...
					if (!found) {
						self.model.bookingRequest.tickets.push({
		...
		setCheckoutStatus: function () {
			if (this.model.bookingRequest.totals !== undefined &&
				this.model.bookingRequest.totals.tickets > 0 &&
				this.model.bookingRequest.email !== undefined &&
				this.model.bookingRequest.email !== '') {
				$('input[name="submit"]').removeAttr('disabled');
			}
			else {
				$('input[name="submit"]').attr('disabled', true);
